...
# Function to recursively check if the given point is in the (hyper)sphere
# First list is the Point to check,
# second is the centre of the sphere, third parameter is the radius
# Returns True or False
def within_range_sphere(point_A, centre, radius):
    total = 0
    for d in range(len(point_A)):
        total = total + (point_A[d]-centre[d])**2
    if total <= radius**2:
        return True
    else:
        return False
...
# Uses MinMax, generate before
def fully_contained(minmax1, minmax2):
    for d in range(len(minmax1)):
        if not (minmax2[d][0] <= minmax1[d][0]
                and minmax2[d][1] >= minmax1[d][1]):
            return False
    return True
...
def search_KD_tree(v, start_point, end_point, results):
    ...
    if v.left:
        ...
        if (fully_contained(minmax1, minmax2)):
            results = results + v.left.get_offsprings()
        # Intersect
        elif(intersect(minmax1, minmax2)):
            results = (
                results
    ...
    if v.right:
        ...
        # Fully contained within goes here
        if (fully_contained(minmax1, minmax2)):
            results = results + v.right.get_offsprings()
        # Intersect
        elif (intersect(minmax1, minmax2)):
            results = (
                results
...
if __name__ == "__main__":
    ...
    for _ in range(no_queries):
        ...
        for i in range(no_dimensions):
            centre.append(int(input_point[i]))
        radius = int(input_point[no_dimensions])

        # Now we create the circumscribed hypercube around the hypersphere
        start = []
        for i in range(no_dimensions):
            start.append(centre[i]-radius)

        end = []
        for i in range(no_dimensions):
            end.append(centre[i]+radius)

        # And call the function on the hypercube,
        # will include unwanted values for now
        points = search_KD_tree(root, start, end, [])
        for p in points:
            if within_range_sphere(p, centre, radius):
                # Print elements that are in the hypersphere
